...
function itemSelector<KeyType, ValueType>(key: KeyType, map: (value: ValueType) => KeyType): (value: ValueType) => boolean {
...
export class ArrayMap<KeyType, ValueType> implements Map<KeyType, ValueType> {
	...
	public constructor(values: Iterable<ValueType>, mapper: (value: ValueType) => KeyType) {
	...
	public delete(key: KeyType): boolean {
		let index = this._values.findIndex(itemSelector(key, this._map));
		if (index >= 0) {
			this._values.splice(index, 1);
			return true;
		} else {
